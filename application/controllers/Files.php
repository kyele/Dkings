	...
	class Files extends REST_Controller
	{
		...
		public function index_post() {
			...
			if ( !$_FILES && !$_POST ) {
				...
				$copia 				= imagecreatetruecolor( 124 , 75 );
				...
				imagecopyresampled( $copia , $original , 0 , 0 , 0 , 0 , 125 , 75 , $ancho_originial , $alto_originial );

				//exportar imagen/guardar imagen
				switch( $tipo ) {
					case 'image/jpg':
					case 'image/jpeg':
						if( imagejpeg( $copia , "images/gallery/miniatura/".$nombre , 60 ) ){
							$resultado = true;
						} else {
							$resultado = false;
						}
					break;
					case 'image/png':
						if( imagepng( $copia , "images/gallery/miniatura/".$nombre , 6 ) ){
							$resultado = true;
						} else {
							$resultado = false;
						}
					break;
					case 'image/gif':
						if( imagegif( $copia , "images/gallery/miniatura/".$nombre , 60 ) ){
							$resultado = true;
						} else {
							$resultado = false;
						}
